...
const MERGE_IMAGE_DISPLAY_NAME = '产品控制融合专家'
...
function mergeImageRole(file = {}, index = -1) {
  const name = String(file.originalName || file.fileName || '')
  const angleProductRole = [
    { role: 'angle01FrontProduct', pattern: /merge-angle01-(?:front|inner)-product-/i },
    { role: 'angle02InnerSideProduct', pattern: /merge-angle02-inner-product-/i },
    { role: 'angle03InnerSideProduct', pattern: /merge-angle03-inner-product-/i },
    ...
    .map(({ role, pattern }) => {
      ...
      return match ? { role, index: match.index } : null
    })
    .filter(Boolean)
    .sort((a, b) => a.index - b.index)[0]
  if (angleProductRole) return angleProductRole.role
  ...
  if (/merge-angle-hand-shoe-prelayout-/i.test(name)) return 'angleShoePrelayout'
  if (/merge-angle-foot-side-reference-/i.test(name)) return 'angleFootSideReference'
  if (/merge-angle-leg-action-reference-/i.test(name)) return 'angleLegActionReference'
...
function describeMergeImageUploadedFiles(files = []) {
  ...
  const roleLabels = {
    ...
    angleShoePrelayout: 'hand-angle product shoe prelayout reference: visual reference for product-shoe toe/heel direction, placement, scale, and rotation only',
    ...
    angleFootSideReference: 'angle 17 foot-side reference: controls only the worn foot ankle side and the visible no-buckle side of the worn shoe',
    angleLegActionReference: 'angle 2 leg-action reference: controls only lower-leg pose, crossed-leg relationship, foot/ankle direction, first-person oblique camera feeling, and close crop; never copy shoes, black/transparent background, clothing color, lighting, or style; final background comes only from the uploaded background image',
    angle01FrontProduct: 'angle 1 inner-side product reference (Product 2 / mergeRef=angle-1-inner-side-product-shoe): only for angle 1, Product 1 controls shoe outer sides on lateral/outer ankles and Product 2 controls shoe inner sides on medial/inner ankles; do not generate same-side shoes',
    angle02InnerSideProduct: 'angle 2 inner-side product reference (Product 2 / mergeRef=angle-2-inner-side-product-shoe): only for angle 2, this is the authoritative appearance source for the far-left and far-right inner shoe sides; main product-shoe reference controls the middle-facing outer shoe sides; do not create an extra shoe',
    angle03InnerSideProduct: 'angle 3 inner-side product reference (Product 2 / mergeRef=angle-3-inner-side-product-shoe): only for angle 3, this is the authoritative appearance source for the two middle-facing inner shoe sides; main product-shoe reference controls the outside-facing outer shoe sides; do not create an extra shoe',
...
function buildMergeImageGenerationReferenceUrls(files = [], angleSource = '') {
  ...
  const urlsFor = (role) => (map.get(role) || []).map((file) => file.url).filter(Boolean)
  const angleControlUrl = firstUrl('angleControl')
  const angleShoePrelayoutUrl = firstUrl('angleShoePrelayout')
  const angleUrl = firstUrl('angle')
  const modelUrl = firstUrl('model')
  const productUrl = firstUrl('product')
  const backgroundUrl = firstUrl('background')
  const angleFootSideReferenceUrl = firstUrl('angleFootSideReference')
  const angleLegActionReferenceUrl = firstUrl('angleLegActionReference')
  const productUrls = urlsFor('product')
  const angle01FrontProductUrls = urlsFor('angle01FrontProduct')
  const angle02InnerSideProductUrls = urlsFor('angle02InnerSideProduct')
  const angle03InnerSideProductUrls = urlsFor('angle03InnerSideProduct')
  ...
  const angleReferences = angleControlUrl
    ? [
        ...
        tagReferenceUrl(angleShoePrelayoutUrl, 'hand-angle-product-shoe-prelayout'),
  ...
  const summary = [
    ...
    tagReferenceUrl(productUrl, 'product-shoe'),
    ...productUrls.slice(1).map((url, index) => tagReferenceUrl(url, `product-shoe-extra-${index + 2}`)),
    ...angle01FrontProductUrls.map((url) => tagReferenceUrl(url, 'angle-1-inner-side-product-shoe')),
    ...angle02InnerSideProductUrls.map((url) => tagReferenceUrl(url, 'angle-2-inner-side-product-shoe')),
    ...angle03InnerSideProductUrls.map((url) => tagReferenceUrl(url, 'angle-3-inner-side-product-shoe')),
...
function buildMergeImageLockInstruction(files = [], angleSource = '') {
  ...
  const extraProductReferenceLock = hasExtraProductReferences
    ...
    : ''
  const angle01TwoProductLock = roleMap.has('angle01FrontProduct')
    ? 'ANGLE 1 TWO-PRODUCT FRONT/REAR MARKED-SIDE REFERENCE LOCK: this lock applies only when angle 1 receives a second product shoe image. Follow the angle-01 marked reference exactly. The FRONT FOOT / lower-left planted shoe displays the model medial ankle / INNER side and must use Product image 2 / angle-1-inner-side-product-shoe. The REAR FOOT / upper-right lifted shoe displays the model lateral ankle / OUTER side and must use Product image 1 / main product-shoe reference. Do not assign by generic screen side alone. Generate the two worn shoes as a real left-foot/right-foot pair according to this marked front-foot inner-side and rear-foot outer-side relationship. Do not generate two same-side shoes, do not copy Product 1 onto the front-foot inner side, do not copy Product 2 onto the rear-foot outer side, and do not swap inner/outer roles. Product 2 is not a background, model, clothing, angle mask, hand-held shoe, third shoe, or generic supplementary view. Keep the angle-01 walking-left pose, front foot lower-left planted position, rear foot upper-right lifted-heel position, toe direction, heel lift, lower-body crop, and close shoe-focused composition.'
    : ''
  const angle01TwoProductShoeLock = roleMap.has('angle01FrontProduct')
    ? 'ANGLE 1 TWO-PRODUCT SHOE LOCK: Product image 2 / mergeRef=angle-1-inner-side-product-shoe controls only the FRONT FOOT / lower-left planted shoe visible INNER side at the medial ankle. Product image 1 / mergeRef=product-shoe controls only the REAR FOOT / upper-right lifted shoe visible OUTER side at the lateral ankle. Preserve each assigned side-specific shoe shape, color, material, texture, strap/buckle/lace, opening, side wall, toe, heel, sole, stitching, lining, shine, and proportions from its assigned product reference. The angle-library image and yellow mask are only placement, angle, scale, perspective, and occlusion references; never use them as shoe exterior appearance, silhouette, color, material, style, shape, sole, heel, toe, strap, lace, buckle, or design references. Do not use Product 1 appearance for the front-foot inner side and do not use Product 2 appearance for the rear-foot outer side.'
    : ''
  const angle02TwoProductLock = roleMap.has('angle02InnerSideProduct')
    ? 'ANGLE 2 TWO-PRODUCT INNER/OUTER SIDE REFERENCE LOCK: this lock applies only when angle 2 receives a second product shoe image. Product image 1 / main product-shoe reference / mergeRef=product-shoe controls only the shoe OUTER side appearance. Product image 2 / angle-2-inner-side-product-shoe controls only the shoe INNER side appearance. Angle 2 remains the seated first-person crossed-leg worn-shoe pose: generate both shoes worn on feet, keep the close shoe-focused crop, and do not add extra shoes. In the selected angle-02 reference, the two sides close to the center gap are OUTER sides and must use Product 1; the far-left side of the left/lower shoe and the far-right side of the right/upper shoe are INNER sides and must use Product 2. Do not swap inner/outer roles, and do not treat Product 2 as background, model, foot, leg, or generic supplementary view.'
    : ''
  const angle03TwoProductLock = roleMap.has('angle03InnerSideProduct')
    ? 'ANGLE 3 TWO-PRODUCT INNER/OUTER SIDE REFERENCE LOCK: this lock applies only when angle 3 receives a second product shoe image. Product image 1 / main product-shoe reference / mergeRef=product-shoe controls only the shoe OUTER side appearance. Product image 2 / angle-3-inner-side-product-shoe controls only the shoe INNER side appearance. Angle 3 is a hand-only product display: generate no feet, no legs, no ankles, no toes, and no worn shoes. The two displayed shoes must stay close together like the angle-03 reference; the sides touching or nearly touching in the middle are INNER sides and must use Product 2. The left outside edge of the left shoe and the right outside edge of the right shoe are OUTER sides and must use Product 1. Do not swap inner/outer roles, do not turn Product 2 into a third shoe, and do not treat Product 2 as a background, model, foot, leg, or generic supplementary view.'
    : ''
  const angle04TwoProductLock = roleMap.has('angle04GroundProduct')
    ? 'ANGLE 4 TWO-PRODUCT FOOT-ROLE REFERENCE LOCK: this lock applies only when angle 4 receives a second product shoe image. Product image 1 / main product-shoe reference / mergeRef=product-shoe controls only the raised/lifted-foot worn shoe marked 图一 in the angle-04 reference. Product image 2 / angle-4-ground-foot-product-shoe controls only the grounded/standing-foot worn shoe marked 图二 in the angle-04 reference and is the authoritative appearance source for that grounded-foot shoe. Do not swap roles: raised/lifted foot uses Product 1, grounded/standing foot uses Product 2. Both are worn shoes on their assigned feet, not hand-held shoes, not standalone display shoes, and not supplementary views of one shoe. Keep the angle-04 pose, lifted-foot/grounded-foot positions, yellow shoe placement, toe/heel direction, lower-body crop, and close shoe-focused composition.'
    : ''
  ...
  if (hasAngleControl) {
    return [
      ...
      roleMap.has('product')
        ? roleMap.has('angle04GroundProduct')
          ? 'ANGLE 4 TWO-PRODUCT SHOE LOCK: the main product-shoe reference / mergeRef=product-shoe controls only the raised/lifted-foot worn shoe marked 图一. Product image 2 / mergeRef=angle-4-ground-foot-product-shoe controls only the grounded/standing-foot worn shoe marked 图二. Preserve each shoe shape, color, material, texture, strap/buckle/lace, toe, heel, sole, stitching, lining, shine, and proportions from its assigned product reference. Do not use Product 1 appearance for the grounded-foot shoe and do not use Product 2 appearance for the raised-foot shoe.'
          : roleMap.has('angle01FrontProduct')
          ? angle01TwoProductShoeLock
          : roleMap.has('angle05InnerSideProduct')
          ? 'ANGLE 5 TWO-PRODUCT SHOE LOCK: the main product-shoe reference / mergeRef=product-shoe controls only shoe OUTER sides on lateral/outer ankles. Product image 2 / mergeRef=angle-5-inner-side-product-shoe controls only shoe INNER sides on medial/inner ankles. Preserve each side-specific shoe shape, color, material, texture, strap/buckle/lace, opening, side wall, toe, heel, sole, stitching, lining, shine, and proportions from its assigned product reference. Do not use Product 1 appearance for inner-ankle sides and do not use Product 2 appearance for outer-ankle sides.'
          : roleMap.has('angle06HandProduct')
          ? 'ANGLE 6 TWO-PRODUCT SHOE LOCK: the main product-shoe reference / mergeRef=product-shoe controls only the worn shoe on the single visible foot. Product image 2 / mergeRef=angle-6-hand-held-product-shoe controls only the left/upper-left hand-held display shoe and is the authoritative appearance source for that hand-held shoe. Preserve each assigned shoe shape, color, material, texture, laces, eyelets, tongue, perforations, side panels, stitching, sole edge, heel patch, buckle/clasp/ring/strap hardware when visible, toe, heel, shine, and proportions. Do not simplify Product 2 into a blank no-detail side.'
          : 'PRODUCT SHOE LOCK: the uploaded product shoe is the only and 100% authoritative shoe identity source. Preserve shoe shape, color, material, texture, strap/buckle/lace, toe, heel, sole, stitching, lining, shine, and proportions.'
      ...
      angle01TwoProductLock,
      angle02TwoProductLock,
  ...
  return [
    ...
    roleMap.has('product')
      ? roleMap.has('angle04GroundProduct')
        ? 'ANGLE 4 TWO-PRODUCT SHOE LOCK: the main product-shoe reference / mergeRef=product-shoe controls only the raised/lifted-foot worn shoe marked 图一. Product image 2 / mergeRef=angle-4-ground-foot-product-shoe controls only the grounded/standing-foot worn shoe marked 图二. Preserve each shoe shape, color, material, texture, strap/buckle/lace, toe, heel, sole, stitching, lining, shine, and proportions from its assigned product reference. The angle-library image and yellow mask are only placement, angle, scale, perspective, and occlusion references; never use them as shoe exterior appearance, silhouette, color, material, style, shape, sole, heel, toe, strap, lace, buckle, or design references. Do not use Product 1 appearance for the grounded-foot shoe and do not use Product 2 appearance for the raised-foot shoe.'
        : roleMap.has('angle01FrontProduct')
        ? angle01TwoProductShoeLock
        : roleMap.has('angle05InnerSideProduct')
        ? 'ANGLE 5 TWO-PRODUCT SHOE LOCK: the main product-shoe reference / mergeRef=product-shoe controls only shoe OUTER sides on lateral/outer ankles. Product image 2 / mergeRef=angle-5-inner-side-product-shoe controls only shoe INNER sides on medial/inner ankles. Preserve each side-specific shoe shape, color, material, texture, strap/buckle/lace, opening, side wall, toe, heel, sole, stitching, lining, shine, and proportions from its assigned product reference. The angle-library image and yellow mask are only placement, angle, scale, perspective, and occlusion references; never use them as shoe exterior appearance, silhouette, color, material, style, shape, sole, heel, toe, strap, lace, buckle, or design references. Do not use Product 1 appearance for inner-ankle sides and do not use Product 2 appearance for outer-ankle sides.'
        : roleMap.has('angle06HandProduct')
        ? 'ANGLE 6 TWO-PRODUCT SHOE LOCK: the main product-shoe reference / mergeRef=product-shoe controls only the worn shoe on the single visible foot. Product image 2 / mergeRef=angle-6-hand-held-product-shoe controls only the left/upper-left hand-held display shoe and is the authoritative appearance source for that hand-held shoe. Preserve each assigned shoe shape, color, material, texture, laces, eyelets, tongue, perforations, side panels, stitching, sole edge, heel patch, buckle/clasp/ring/strap hardware when visible, toe, heel, shine, and proportions. Do not simplify Product 2 into a blank no-detail side. The angle-library image and yellow mask are only placement, angle, scale, perspective, and occlusion references; never use them as shoe exterior appearance, silhouette, color, material, style, shape, sole, heel, toe, strap, lace, buckle, or design references. Do not let the main product-shoe reference become the hand-held display shoe appearance.'
        : 'PRODUCT SHOE LOCK: the uploaded product shoe is the only and 100% authoritative shoe identity source. Preserve shoe shape, color, material, texture, strap/buckle/lace, toe, heel, sole, stitching, lining, shine, and proportions. The angle-library image and yellow mask are only placement, angle, scale, perspective, and occlusion references; never use them as shoe exterior appearance, silhouette, color, material, style, shape, sole, heel, toe, strap, lace, buckle, or design references.'
      : 'Missing product shoe image; do not invent a product shoe.',
    roleMap.has('background') ? 'BACKGROUND LOCK: the uploaded background is the only environment source. Preserve wall, floor, baseboard, space, light direction, color temperature, and shadow mood.' : 'Missing background image; do not invent a separate scene.',
...
function buildMergeImageReferenceOrderInstruction(files = [], angleSource = '') {
  ...
  const roleNames = {
    ...
    angleShoePrelayout: 'hand-angle product shoe prelayout reference',
    angle: 'angle / semantic mask reference',
    angle01FrontProduct: 'angle 1 inner-side product reference (Product 2 / mergeRef=angle-1-inner-side-product-shoe)',
    angle02InnerSideProduct: 'angle 2 inner-side product reference (Product 2 / mergeRef=angle-2-inner-side-product-shoe)',
    angle04GroundProduct: 'angle 4 ground-foot product reference (Product 2 / mergeRef=angle-4-ground-foot-product-shoe)',
  ...
  const hasAngleShoePrelayout = ordered.some(({ role }) => role === 'angleShoePrelayout')
  const hasAngle01FrontProduct = ordered.some(({ role }) => role === 'angle01FrontProduct')
  const hasAngle02InnerSideProduct = ordered.some(({ role }) => role === 'angle02InnerSideProduct')
  const hasAngle03InnerSideProduct = ordered.some(({ role }) => role === 'angle03InnerSideProduct')
  ...
  return [
    ...
    ...lines,
    hasAngleControl
      ? hasAngleShoePrelayout
        ? hasModel
          ? hasAngle01FrontProduct
            ? 'Important: the image model receives reordered references in this exact order: image 1 is the cleaned angle control mask, image 2 is the hand-angle product shoe prelayout reference, image 3 is the original uploaded angle reference, image 4 is the background environment, image 5 is model outfit and body-limb reference, image 6 is the main product-shoe reference for shoe OUTER sides on lateral/outer ankles, image 7 is the angle-1-inner-side-product-shoe reference / Product 2 for shoe INNER sides on medial/inner ankles.'
            : 'Important: the image model receives reordered references in this exact order: image 1 is the cleaned angle control mask, image 2 is the hand-angle product shoe prelayout reference, image 3 is the original uploaded angle reference, image 4 is the background environment, image 5 is model outfit and body-limb reference, image 6 is product shoe.'
          : hasAngle01FrontProduct
            ? 'Important: the image model receives reordered references in this exact order: image 1 is the cleaned angle control mask, image 2 is the hand-angle product shoe prelayout reference, image 3 is the original uploaded angle reference, image 4 is the background environment, image 5 is the main product-shoe reference for shoe OUTER sides on lateral/outer ankles, image 6 is the angle-1-inner-side-product-shoe reference / Product 2 for shoe INNER sides on medial/inner ankles.'
            : 'Important: the image model receives reordered references in this exact order: image 1 is the cleaned angle control mask, image 2 is the hand-angle product shoe prelayout reference, image 3 is the original uploaded angle reference, image 4 is the background environment, image 5 is product shoe.'
        : hasModel
          ? hasAngle01FrontProduct
            ? 'Important: the image model receives reordered references in this exact order: image 1 is the cleaned angle control mask, image 2 is the original uploaded angle reference, image 3 is the background environment, image 4 is model outfit and body-limb reference, image 5 is the main product-shoe reference for shoe OUTER sides on lateral/outer ankles, image 6 is the angle-1-inner-side-product-shoe reference / Product 2 for shoe INNER sides on medial/inner ankles.'
          ...
          : hasAngle01FrontProduct
            ? 'Important: the image model receives reordered references in this exact order: image 1 is the cleaned angle control mask, image 2 is the original uploaded angle reference, image 3 is the background environment, image 4 is the main product-shoe reference for shoe OUTER sides on lateral/outer ankles, image 5 is the angle-1-inner-side-product-shoe reference / Product 2 for shoe INNER sides on medial/inner ankles.'
      ...
      : hasAngle04GroundProduct
        ? hasModel
          ...
          : `Important: the image model receives reordered references in this exact order: image 1 is the ${hasRealPhotoAngle ? 'real photo angle reference' : 'angle mask'}, image 2 is the background environment, image 3 is the main product-shoe reference for the raised/lifted-foot worn shoe marked 图一, image 4 is the angle-4-ground-foot-product-shoe reference / Product 2 for the grounded/standing-foot worn shoe marked 图二.`
      : hasAngle05InnerSideProduct
        ? hasModel
          ? `Important: the image model receives reordered references in this exact order: image 1 is the ${hasRealPhotoAngle ? 'real photo angle reference' : 'angle mask'}, image 2 is the background environment, image 3 is model outfit and body-limb reference, image 4 is the main product-shoe reference for shoe OUTER sides on lateral/outer ankles, image 5 is the angle-5-inner-side-product-shoe reference / Product 2 for shoe INNER sides on medial/inner ankles.`
          : `Important: the image model receives reordered references in this exact order: image 1 is the ${hasRealPhotoAngle ? 'real photo angle reference' : 'angle mask'}, image 2 is the background environment, image 3 is the main product-shoe reference for shoe OUTER sides on lateral/outer ankles, image 4 is the angle-5-inner-side-product-shoe reference / Product 2 for shoe INNER sides on medial/inner ankles.`
      : hasAngle06HandProduct
        ? hasModel
          ? `Important: the image model receives reordered references in this exact order: image 1 is the ${hasRealPhotoAngle ? 'real photo angle reference' : 'angle mask'}, image 2 is the background environment, image 3 is model outfit and body-limb reference, image 4 is the main product-shoe reference for the worn shoe, image 5 is Product 2 / angle-6-hand-held-product-shoe reference for the hand-held display shoe.`
          : `Important: the image model receives reordered references in this exact order: image 1 is the ${hasRealPhotoAngle ? 'real photo angle reference' : 'angle mask'}, image 2 is the background environment, image 3 is the main product-shoe reference for the worn shoe, image 4 is Product 2 / angle-6-hand-held-product-shoe reference for the hand-held display shoe.`
        : hasModel
    ...
    hasAngle01FrontProduct
      ? 'Angle 1 two-product reference: follow the angle-01 marked front/rear foot labels. The front/lower-left planted shoe visible side is the INNER side on the medial ankle and must use Product 2 / angle-1-inner-side-product-shoe. The rear/upper-right lifted shoe visible side is the OUTER side on the lateral ankle and must use Product 1 / main product-shoe reference. Do not generate two same-side shoes, do not swap front/rear assignments, do not swap inner/outer sides, and never treat Product 2 as a hand-held, standalone, or supplementary reference.'
      : '',
    hasAngle02InnerSideProduct
      ? 'Angle 2 two-product reference: the main product-shoe reference controls the middle-facing OUTER shoe sides; the angle-2-inner-side-product-shoe reference / Product 2 is the authoritative appearance source for the far-left and far-right INNER shoe sides. Keep the angle-2 seated first-person crossed-leg relationship: two worn shoes on both feet, center-facing sides are outer sides, far outside edges are inner sides. Do not add extra shoes, do not swap inner/outer sides, and never treat Product 2 as a generic supplementary reference.'
      : '',
    hasAngle03InnerSideProduct
    ...
    hasAngle04GroundProduct
      ...
      : '',
    hasAngle05InnerSideProduct
      ? 'Angle 5 two-product reference: the main product-shoe reference / Product 1 controls the OUTER shoe sides on the lateral malleolus / outer-ankle side of each foot; the angle-5-inner-side-product-shoe reference / Product 2 controls the INNER shoe sides on the medial malleolus / inner-ankle side of each foot. Follow the angle-05 labels for visible inner/outer ankle sides. Do not generate two same-side shoes, do not swap inner/outer sides, and never treat Product 2 as a hand-held, standalone, or supplementary reference.'
      : '',
    hasAngle06HandProduct
      ? 'Angle 6 two-product reference: the main product-shoe reference controls the worn shoe on the single visible foot; Product 2 / angle-6-hand-held-product-shoe reference is the authoritative appearance source for the left/upper-left hand-held display shoe. The hand-held display shoe exterior must match Product 2 actual visible details, including laces, eyelets, tongue, perforations, stitching, side panels, sole edge, heel patch, visible hardware, material texture, and proportions. This overrides generic product-shoe identity rules for that hand-held shoe only. Do not swap the two product roles and never add an extra shoe from the angle-6-hand-held-product-shoe reference.'
      : '',
    hasAngleControl
      ? hasAngleShoePrelayout
        ? hasModel
          ? hasAngle01FrontProduct
            ? 'Use image 1 cleaned angle control first to determine exact pose/layout, final main shoe count, region roles, and coordinates. Use image 2 hand-angle product shoe prelayout as the strongest visual reference for real product-shoe toe/front direction, heel direction, rotation, placement, and scale inside each S region. Use image 3 original angle reference only to verify broad pose/action and occlusion. Use image 4 background reference for environment structure. Use image 5 model reference only for outfit, skin tone, and body-limb styling; ignore background, props, watermark, and shoes. For angle 1 two-product mode, use image 6 only for OUTER shoe sides on lateral/outer ankles and image 7 only for INNER shoe sides on medial/inner ankles.'
            : 'Use image 1 cleaned angle control first to determine exact pose/layout, final main shoe count, region roles, and coordinates. Use image 2 hand-angle product shoe prelayout as the strongest visual reference for real product-shoe toe/front direction, heel direction, rotation, placement, and scale inside each S region. Use image 3 original angle reference only to verify broad pose/action and occlusion. Use image 4 background reference for environment structure. Use image 5 model reference only for outfit, skin tone, and body-limb styling; ignore background, props, watermark, and shoes. Use image 6 product reference for shoe identity.'
          : hasAngle01FrontProduct
            ? 'Use image 1 cleaned angle control first to determine exact pose/layout, final main shoe count, region roles, and coordinates. Use image 2 hand-angle product shoe prelayout as the strongest visual reference for real product-shoe toe/front direction, heel direction, rotation, placement, and scale inside each S region. Use image 3 original angle reference only to verify broad pose/action and occlusion. Use image 4 background reference for environment structure. For angle 1 two-product mode, use image 5 only for OUTER shoe sides on lateral/outer ankles and image 6 only for INNER shoe sides on medial/inner ankles.'
        ...
        : hasModel
          ? hasAngle01FrontProduct
            ...
            : 'Use image 1 cleaned angle control first and most strongly to determine exact pose/layout, final main shoe count, yellow shoe-region placement/angle/scale, worn-shoe supplements around blue feet, and each blue region classification as LEG/FOOT/ANKLE, HAND/HANDWRIST/FINGER, or minor detail. Use image 2 original angle reference only to verify fine pose and occlusion. Use image 3 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 4 model reference only for outfit, skin tone, and body-limb styling; ignore background, props, watermark, and shoes. Use image 5 product reference for shoe identity.'
          : hasAngle01FrontProduct
            ? 'Use image 1 cleaned angle control first and most strongly to determine exact pose/layout, final main shoe count, yellow shoe-region placement/angle/scale, worn-shoe supplements around blue feet, and each blue region classification as LEG/FOOT/ANKLE, HAND/HANDWRIST/FINGER, or minor detail. Use image 2 original angle reference only to verify fine pose and occlusion. Use image 3 background reference for all environment, wall, floor, baseboard, light, and shadows. For angle 1 two-product mode, use image 4 only for OUTER shoe sides on lateral/outer ankles and image 5 only for INNER shoe sides on medial/inner ankles.'
            : 'Use image 1 cleaned angle control first and most strongly to determine exact pose/layout, final main shoe count, yellow shoe-region placement/angle/scale, worn-shoe supplements around blue feet, and each blue region classification as LEG/FOOT/ANKLE, HAND/HANDWRIST/FINGER, or minor detail. Use image 2 original angle reference only to verify fine pose and occlusion. Use image 3 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 4 product reference for shoe identity.'
      : hasModel
        ? hasAngle01FrontProduct
          ? hasRealPhotoAngle
            ? 'Use image 1 real photo angle reference first and most strongly to determine pose/layout, real camera viewpoint, crop, shoe count, shoe placement, foot/ankle relationship, scale, perspective, and occlusion. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 3 model reference only for outfit, skin tone, and body-limb styling; ignore background, props, watermark, and shoes. For angle 1 two-product mode: use image 4 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 5 angle-1-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not let image 4 override the inner-ankle side.'
            : 'Use image 1 angle mask first and most strongly to determine pose/layout, yellow shoe-region count/placement/angle/scale, walking-left foot positions, toe direction, heel lift, and each blue region silhouette as leg/ankle/foot. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 3 model reference only for outfit, skin tone, and body-limb styling; ignore background, props, watermark, and shoes. For angle 1 two-product mode: use image 4 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 5 angle-1-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not let image 4 override the inner-ankle side.'
          : hasAngle05InnerSideProduct
          ? hasRealPhotoAngle
            ...
            : 'Use image 1 angle mask first and most strongly to determine pose/layout, yellow shoe-region count/placement/angle/scale, angle-05 foot positions, toe direction, and each blue region silhouette as leg/ankle/foot. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 3 model reference only for outfit, skin tone, and body-limb styling; ignore background, props, watermark, and shoes. For angle 5 two-product mode: use image 4 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 5 angle-5-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not let image 4 override the inner-ankle side.'
          : hasAngle04GroundProduct
          ? hasRealPhotoAngle
            ? 'Use image 1 real photo angle reference first and most strongly to determine pose/layout, real camera viewpoint, crop, shoe count, shoe placement, body/hand/foot relationship, scale, perspective, and occlusion. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 3 model reference only for outfit, skin tone, and body-limb styling; ignore background, props, watermark, and shoes. Use image 4 main product-shoe reference only for the raised/lifted-foot worn shoe marked 图一. Use image 5 angle-4-ground-foot-product-shoe reference / Product 2 only for the grounded/standing-foot worn shoe marked 图二.'
            : 'Use image 1 angle mask first and most strongly to determine pose/layout, yellow shoe-region count/placement/angle/scale, lifted-foot/grounded-foot roles, and each blue region silhouette as leg/ankle/foot or arm/hand/wrist. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 3 model reference only for outfit, skin tone, and body-limb styling; ignore background, props, watermark, and shoes. Use image 4 main product-shoe reference only for the raised/lifted-foot worn shoe marked 图一. Use image 5 angle-4-ground-foot-product-shoe reference / Product 2 only for the grounded/standing-foot worn shoe marked 图二.'
          : hasAngle06HandProduct
          ? hasRealPhotoAngle
            ? 'Use image 1 real photo angle reference first and most strongly to determine pose/layout, real camera viewpoint, crop, shoe count, shoe placement, body/hand/foot relationship, scale, perspective, and occlusion. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 3 model reference only for outfit, skin tone, and body-limb styling; ignore background, props, watermark, and shoes. Use image 4 main product-shoe reference only for the worn shoe identity. Use image 5 angle-6-hand-held-product-shoe reference / 产品2 only for the hand-held display shoe appearance.'
        ...
        : hasAngle01FrontProduct
          ? hasRealPhotoAngle
            ? 'Use image 1 real photo angle reference first and most strongly to determine pose/layout, real camera viewpoint, crop, shoe count, shoe placement, foot/ankle relationship, scale, perspective, and occlusion. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. For angle 1 two-product mode: use image 3 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 4 angle-1-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not let image 3 override the inner-ankle side.'
            : 'Use image 1 angle mask first and most strongly to determine pose/layout, yellow shoe-region count/placement/angle/scale, walking-left foot positions, toe direction, heel lift, and each blue region silhouette as leg/ankle/foot. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. For angle 1 two-product mode: use image 3 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 4 angle-1-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not let image 3 override the inner-ankle side.'
          : hasAngle05InnerSideProduct
          ? hasRealPhotoAngle
          ...
          : hasAngle04GroundProduct
          ? hasRealPhotoAngle
            ? 'Use image 1 real photo angle reference first and most strongly to determine pose/layout, real camera viewpoint, crop, shoe count, shoe placement, body/hand/foot relationship, scale, perspective, and occlusion. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 3 main product-shoe reference only for the raised/lifted-foot worn shoe marked 图一. Use image 4 angle-4-ground-foot-product-shoe reference / Product 2 only for the grounded/standing-foot worn shoe marked 图二.'
            : 'Use image 1 angle mask first and most strongly to determine pose/layout, yellow shoe-region count/placement/angle/scale, lifted-foot/grounded-foot roles, and each blue region silhouette as leg/ankle/foot or arm/hand/wrist. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 3 main product-shoe reference only for the raised/lifted-foot worn shoe marked 图一. Use image 4 angle-4-ground-foot-product-shoe reference / Product 2 only for the grounded/standing-foot worn shoe marked 图二.'
          : hasAngle06HandProduct
          ? hasRealPhotoAngle
            ? 'Use image 1 real photo angle reference first and most strongly to determine pose/layout, real camera viewpoint, crop, shoe count, shoe placement, body/hand/foot relationship, scale, perspective, and occlusion. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 3 main product-shoe reference only for the worn shoe identity. Use image 4 angle-6-hand-held-product-shoe reference / 产品2 only for the hand-held display shoe appearance.'
            : 'Use image 1 angle mask first and most strongly to determine pose/layout, yellow shoe-region count/placement/angle/scale, and each blue region silhouette as leg/ankle/foot or arm/hand/wrist. Use image 2 background reference for all environment, wall, floor, baseboard, light, and shadows. Use image 3 main product-shoe reference only for the worn shoe identity. Use image 4 angle-6-hand-held-product-shoe reference / 产品2 only for the hand-held display shoe appearance.'
          : hasRealPhotoAngle
...
function buildMergeImageRolePriorityInstruction({ hasAngle01FrontProduct = false, hasAngle02InnerSideProduct = false, hasAngle03InnerSideProduct = false, hasAngle04GroundProduct = false, hasAngle05InnerSideProduct = false, hasAngle06HandProduct = false } = {}) {
  return [
    ...
    'Model reference image: source only for actual outfit category, outfit color, fabric/drape, body-limb styling, skin tone, lower-body proportions, and elegant feeling. Put the model outfit into red regions and matching natural body limbs into blue regions, but keep outfit and clothing secondary to the product shoes. Do not copy model reference shoes, background, wall, floor, props, basket, flowers, watermark, lighting pattern, or scene.',
    hasAngle01FrontProduct
      ? 'Angle 1 two-product shoe references: follow the angle-01 marked front/rear foot labels. The FRONT FOOT / lower-left planted shoe displays the model medial ankle / INNER side and must use the angle-1-inner-side-product-shoe reference / Product 2. The REAR FOOT / upper-right lifted shoe displays the model lateral ankle / OUTER side and must use the main product-shoe reference / Product 1. Preserve side-specific shoe shape, color, material, texture, straps, buckle/clasp/ring visibility, opening, side wall, stitching, lining, and overall proportions from the assigned reference. Yellow regions still control placement, angle, perspective, walking-left pose, foot positions, and occlusion only. Do not generate two same-side shoes, do not swap the front-foot inner side with the rear-foot outer side, and do not use Product 1 for the front-foot inner side.'
      : hasAngle02InnerSideProduct
      ...
      : hasAngle05InnerSideProduct
      ? 'Angle 5 two-product shoe references: the main product-shoe reference / mergeRef=product-shoe controls only the OUTER shoe side appearance on lateral/outer ankles; the angle-5-inner-side-product-shoe reference / Product 2 controls only the INNER shoe side appearance on medial/inner ankles. Preserve side-specific shoe shape, color, material, texture, straps, buckle/clasp/ring visibility, opening, side wall, stitching, lining, and overall proportions from the assigned reference. Yellow regions still control placement, angle, perspective, angle-05 foot positions, and occlusion only. Do not generate two same-side shoes and do not use screen left/right as the product-image assignment rule.'
      : hasAngle06HandProduct
...
function buildMergeImageChineseRecordPrompt({ files = [], angleSource = '', requestedSize = '', apiSize = '', hasModelReference = false } = {}) {
  ...
  const hasAngle01FrontProduct = (roleMap.get('angle01FrontProduct') || []).length > 0
  const hasAngle03InnerSideProduct = (roleMap.get('angle03InnerSideProduct') || []).length > 0
  ...
  return [
    ...
    hasAngle01FrontProduct ? `角度1 产品图二/内侧鞋面参考图：${fileNameForRole('angle01FrontProduct')}` : '',
    hasAngle03InnerSideProduct ? `角度3 产品图二/内侧鞋面参考图：${fileNameForRole('angle03InnerSideProduct')}` : '',
...
function buildStrictPositionLock(angleLayout = '') {
  ...
  if (isHandAngleControl) {
    return [
      '[STRICT HAND-ANGLE COMPOSITION LOCK - NATURAL PHOTO TOLERANCE]',
      '[MUST] Treat the code-generated control image as a fixed X0-X100 / Y0-Y100 composition map, but render it as a natural photograph using the cameraViewByCode / HAND ANGLE CAMERA VIEW BY CODE from the layout, not as a pasted geometric reconstruction.',
      '[MUST] Preserve every S/B/G/R object zone, center relationship, role, scale relationship, overlap, and depth order. Small natural perspective shifts are allowed only when they make the hand, foot, shoe contact, and shadows look photographic.',
      '[MUST] Do not recenter, flip, straighten, or move objects into a generic commercial pose. Keep each object near its required zone below; a large move to another zone fails.',
      ...objectLines,
    ].join('\n').slice(0, 2600)
  }
  return [
...
function buildHardAngleLayoutSummary(angleLayout = '') {
  ...
  const ignoredHeelOrDetailFragmentCount = valueFor('ignoredHeelOrDetailFragmentCount')
  const handAngleNoBlueRegion = valueFor('handAngleNoBlueRegion')
  const handAngleNoValidBlueFootLeg = valueFor('handAngleNoValidBlueFootLeg')
  ...
  const shoeLines = pickLines(/^(SINGLE-FOOT BUCKLE VISUAL VERIFICATION BY CODE|SINGLE-FOOT WHOLE-SHOE SIDE LOCK BY CODE|SINGLE-FOOT INDEPENDENT SHOE MAP BY CODE|SINGLE-FOOT PAIR GUIDE VISUAL RULE|S1 WORN x1|S2 DISPLAY x1|S2 toeDirection|S object explanation|S\d+:|Binding S\d+:|SHOE\/YELLOW MAP)/i, 18)
  ...
  const poseLines = pickLines(/^(AI VISUAL POSE RECOGNITION|HAND ANGLE CAMERA VIEW BY CODE|HAND ANGLE NATURAL PHOTO POSE SUMMARY BY CODE|Use this short natural-language pose summary|S\d+:\s+place a realistic product shoe|Generate a natural .* commercial photo|CODE ROUGH POSE DESCRIPTION|ORIGINAL HAND-ANGLE VISUAL POSE)/i, 12)
  const directionLines = pickLines(/^(HAND ANGLE MASK COLOR BAN|HAND ANGLE CAMERA VIEW BY CODE|HAND ANGLE EYE-LEVEL SIDE-VIEW CAMERA LOCK|HAND ANGLE FIRST-PERSON DOWNWARD CAMERA LOCK|HAND ANGLE TOP-DOWN GROUND-PLANE LOCK|HAND ANGLE PER-SHOE FINAL DIRECTION LOCK BY CODE|S\d+\s+FINAL DIRECTION LOCK|HAND ANGLE NO WORN SHOE LOCK|HAND ANGLE HAND-DISPLAY SHOE LOCK|HAND ANGLE DETACHED BLUE FOOT LOCK|TOE\/HEEL MARKER RULE|BUCKLE\/CLASP MARKER RULE|BUCKLE SIDE HARD LOCK BY CODE|S\d+\s+buckleSide|HAND ANGLE CONTROL SHOE DIRECTION LOCK|HAND ANGLE SHOE COUNT LOCK|HAND ANGLE TWO-SHOE PAIR LOCK|BODY DIRECTION LOCK|CAMERA LOCK|SINGLE-FOOT SINGLE-VISIBLE-FOOT LOCK|SINGLE-FOOT CAMERA DESCRIPTION BY CODE|SINGLE-FOOT CAMERA SIDE LOCK BY CODE|SINGLE-FOOT TOE DIRECTION SAME LOCK BY CODE|SINGLE-FOOT BUCKLE VISUAL VERIFICATION BY CODE|SINGLE-FOOT WHOLE-SHOE SIDE LOCK BY CODE|SINGLE-FOOT SIDE PAIR ANALYSIS BY CODE|SINGLE-FOOT INDEPENDENT SHOE MAP BY CODE|SINGLE-FOOT PAIR GUIDE VISUAL RULE|S1 WORN x1|S2 DISPLAY x1|S2 toeDirection|VISIBLE TWO-SHOE LEFT\/RIGHT PAIR LOCK|VISIBLE MULTI-SHOE PAIRING LOCK|Product-shoe crop rule|Binding rule|Clothing rule)/i, 40)
  const noClothingLock = isHandAngleControl && clothingCount === '0'
  const noHandAngleFeetLock = isHandAngleControl && (/^true$/i.test(handAngleNoBlueRegion) || /^true$/i.test(handAngleNoValidBlueFootLeg))
  const handAngleHeelFragmentLock = isHandAngleControl && ignoredHeelOrDetailFragmentCount
  ...
  const singleFootTopDownGroundLock = isSingleFootControl
    ...
    : ''
  const handAngleSideViewCamera = isHandAngleControl && lines.some((line) => /HAND ANGLE EYE-LEVEL SIDE-VIEW CAMERA LOCK|cameraView=eye-level \/ side-view/i.test(line))
  const handAngleTopDownCamera = isHandAngleControl && lines.some((line) => /HAND ANGLE FIRST-PERSON DOWNWARD CAMERA LOCK|cameraView=first-person downward \/ top-down/i.test(line))
  ...
  return [
    ...
    poseLines.length ? 'AI ROUGH POSE REFERENCE:\n' + poseLines.join('\n') : '',
    ...
    isHandAngleControl && String(handAngleFootLegCount || legFootCount) === '1' ? '[MUST] HAND-ANGLE SINGLE BLUE FOOT LOCK: blueFootLegCount=1 means generate exactly one visible foot/leg/ankle region only. Do not generate a second foot, second leg, second ankle, second sock, second toes, second lower-body limb, or another worn shoe. The model reference may control clothing fabric/style only and must not introduce an extra leg/foot/shoe.' : '',
    isHandAngleControl && String(handAngleShoeObjectCount || mainShoeCount) === '2' && normalizedWornShoes ? `[MUST] HAND-ANGLE TWO-SHOE ROLE LOCK: the final image must contain exactly 2 product shoes total. ${normalizedWornShoes} is/are the only worn shoe(s). Hand-held, hand-supported, and standalone display shoes must remain bare, with no foot, ankle, leg, toes, or sock inside. Do not create any third shoe, second worn shoe, or extra product shoe outside the S layout.` : '',
    handAngleHeelFragmentLock ? `[MUST] HAND-ANGLE SHOE FRAGMENT LOCK: ignoredHeelOrDetailFragmentCount=${ignoredHeelOrDetailFragmentCount}. Small disconnected yellow heel caps, sole slivers, buckles, straps, and shoe-detail fragments are part of the nearest main shoe and must not be generated as separate shoes or increase the shoe count.` : '',
    isHandAngleControl ? '[MUST] HAND-ANGLE MASK COLOR BAN: the final image must not contain any visible hand-angle reference colors or cleaned-control artifacts: no yellow mask shoes, green hands, blue limbs/feet, red clothing blocks, cyan TOE markers, magenta HEEL markers, red direction arrows, orange buckle markers, black mask background, gray guide fills, white labels, grid lines, boxes, colored outlines, color stains, color reflections, tint remnants, or flat color-block shapes. Yellow/S must become the uploaded product shoe only; green/G must become natural skin hand/fingers/wrist; blue/B must become natural skin leg/foot/ankle only when present; red/R must become real clothing fabric from the model reference; cyan/magenta markers, red arrows, and orange buckle markers are only shoe-end/hardware metadata; black/empty must become uploaded background.' : '',
    handAngleSideViewCamera ? '[MUST] HAND-ANGLE CAMERA LOCK: cameraViewByCode says eye-level / side-view. Generate a natural flat/side footwear photo with side-profile legs, shoes, hands, and garment crop as detected. Do not convert it into top-down, overhead, first-person looking-down, flat-lay, or floor-plane POV.' : '',
    handAngleTopDownCamera ? '[MUST] HAND-ANGLE CAMERA LOCK: cameraViewByCode says first-person downward / top-down POV. The camera must look down from above the wearer/model at the shoes, hand, foot/ankle, clothing edge, and floor plane. Do not convert this into an eye-level side-view or standing catalog pose.' : '',
    handAngleTopDownCamera ? '[MUST] HAND-ANGLE TOP-DOWN GROUND-PLANE LOCK: under this downward POV, keep every S shoe, B foot/ankle, G hand, and R clothing crop in the same code coordinates and the same floor-plane relationship. The shoe top surfaces and toe/heel directions must follow the S cyan/magenta markers inside the top-down view; do not rotate the whole scene into a horizontal side profile.' : '',
    isHandAngleControl ? '[MUST] HAND-ANGLE SHOE DIRECTION LOCK: every S/yellow shoe must preserve its code-generated bbox, cyan TOE marker, magenta HEEL marker, red heel-to-toe arrow, heel-to-toe shoeAxis, toe/front coordinate, heel coordinate, and toeDirection. Priority order is: cyan TOE / magenta HEEL / red heel-to-toe arrow first, then S bbox and scale, then hand/foot contact role, then orange buckle/clasp side marker. The cyan/magenta endpoints must be understood as the actual yellow shoe long-axis endpoints, not an abstract bbox centerline. Rotate the uploaded product shoe from magenta HEEL toward cyan TOE to match the S/yellow toeDirection. Do not use the product image original direction when it conflicts with the S/yellow direction. If one S is worn on a blue/B foot and another S is display or hand-supported, align the display/hand-supported S with the worn shoe while keeping its TOE/HEEL markers on that shoe own yellow long axis. Do not fix left/right pairing by reversing a shoe toe direction or moving the shoe away from its bbox. If the buckle/clasp side appears to conflict with toe/heel direction, keep the toe/heel direction and place the hardware on the corresponding outer side without rotating or relocating the shoe.' : '',
    isHandAngleControl && handAngleShoeObjectCount ? `[MUST] HAND-ANGLE SHOE COUNT LOCK: generate exactly ${handAngleShoeObjectCount} visible product shoe object(s), one for each S/yellow object in the SHOE/YELLOW MAP. Do not collapse multiple S objects into one shoe. Do not omit standalone display shoes. Do not invent extra shoes beyond mainShoeObjectCount.` : '',
    isHandAngleControl && String(handAngleShoeObjectCount) === '2' ? '[MUST] HAND-ANGLE TWO-SHOE PAIR LOCK: mainShoeObjectCount=2 means the two visible product shoes must be one real left-shoe/right-shoe pair, not two same-side copies. Buckles, clasps, rings, strap fasteners, buckle holes, strap anchor points, side seams, toe-box curve, opening shape, inner wall, outer wall, and heel counter must show mirrored left/right structure. Buckles/clasps/rings must follow each S orange buckle marker and must not appear on the same canvas side or same shoe side on both shoes. Preserve each S/yellow toeDirection; do not reverse toe/front direction to fake left/right pairing.' : '',
    noHandAngleFeetLock ? '[MUST] HAND-ANGLE NO-BLUE NO-FEET LOCK: handAngleNoBlueRegion=true or handAngleNoValidBlueFootLeg=true means this hand-angle reference has no valid blue leg/foot/ankle component. Do not generate any foot, ankle, leg, toes, skin limb, worn foot, or lower body. All yellow/S shoes must remain bare display shoes or green/G hand-supported shoes only. Red/R clothing may only become cropped clothing fabric and must not introduce legs, feet, ankles, toes, skin limbs, or lower-body anatomy.' : '',
    ...
    isHandAngleControl && /^none$/i.test(String(normalizedWornShoes || '')) ? '[MUST] HAND-ANGLE NO WORN SHOE LOCK: wornShoes=none means no product shoe is worn by any foot. Every S shoe must remain a bare display shoe. Do not insert foot, sock, toes, ankle, leg, or skin into any S shoe, and do not create a try-on or wearing action.' : '',
    isHandAngleControl && /^none$/i.test(String(normalizedWornShoes || '')) && handShoes && !/^none$/i.test(String(handShoes || '')) ? `[MUST] HAND-ANGLE HAND-DISPLAY SHOE LOCK: handheldOrHandSupportedShoes=${handShoes}. These S shoes are bare display product shoes held or supported by green/G hand regions only. The final image should read as hands holding/supporting display shoes, not feet wearing shoes.` : '',
    isHandAngleControl && /^none$/i.test(String(normalizedWornShoes || '')) && String(handAngleFootLegCount || legFootCount) === '1' ? '[MUST] HAND-ANGLE DETACHED BLUE FOOT LOCK: because wornShoes=none, the single blue/B foot-leg region is not a wearing foot. If visible, it may appear only as a secondary cropped lower-leg/sock/foot posture from the reference; it must not overlap, enter, touch, or wear any product shoe.' : '',
    normalizedWornShoes && !/^none$/i.test(String(normalizedWornShoes)) ? (isSingleFootControl
  ...
  return summary.slice(0, isHandAngleControl ? 9000 : 5200)
}
...
function buildHandAngleExposureInstruction() {
  return [
    '[HAND-ANGLE EXPOSURE AND COLOR PRIORITY - READ FIRST]',
    '[MUST] For the angle-reference-with-hands channel, the final image must be a bright high-key ecommerce footwear photograph with clean exposure, clear whites, healthy contrast, natural warm skin tone, and no dark gray cast.',
    '[MUST] Image 1 cleaned hand-angle control and image 2 original color-block hand-angle reference are layout/pose/count/direction metadata only. They must not influence final exposure, brightness, contrast, saturation, color temperature, shadow mood, background darkness, or overall color grading.',
    '[MUST NOT] Do not copy or inherit black mask background, gray guide fills, gray wash, low-key contrast, muted saturation, or flat control-map lighting from the hand-angle references.',
    '[MUST] Use the uploaded background only for wall/floor/baseboard geometry, texture, and broad environment placement. If the background is beige, gray, dim, or low-contrast, brighten it into a clean premium commercial high-key version while preserving the environment structure.',
    '[MUST] Product shoe color and material must stay fresh, clean, and optically separated from the floor/background. Avoid beige-on-beige muddiness, dusty gray shadows, underexposure, haze, and low-saturation indoor cast.',
  ].join('\n')
}
...
function compactProjectImage(image = {}, light = true) {
  const prompt = image.prompt || ''
  const compactPrompt = image.compactPrompt || ''
  const debugFinalPrompt = image.debugFinalPrompt || ''
  ...
  return {
    ...
    referenceImageUrls: Array.isArray(image.referenceImageUrls) ? image.referenceImageUrls : [],
    favorite: Boolean(image.favorite),
    createdAt: image.createdAt || '',
    hasPrompt: Boolean(prompt || compactPrompt || debugFinalPrompt),
  }
}

async function listProjects({ limit = 50, offset = 0, light = true } = {}) {
  ...
  const projects = []
  let validProjectIndex = 0
  for (const { entry } of projectEntries) {
    const projectPath = path.join(projectsRoot, entry.name)
    const project = await readJson(path.join(projectPath, 'project.json'), null)
    if (!project) continue
    if (validProjectIndex < normalizedOffset) {
      validProjectIndex += 1
      continue
    }
    if (projects.length >= normalizedLimit) break
    validProjectIndex += 1
    ...
    projects.push({
      ...
      images: [...byUrl.values()]
        ...
        .map((image) => compactProjectImage(image, light)),
    })
  }
...
app.get('/api/projects', async (req, res, next) => {
  try {
    ...
    const projects = await listProjects({ limit: limit + 1, offset, light: true })
    res.json({
      projects: projects.slice(0, limit),
      hasMore: projects.length > limit,
      nextOffset: offset + Math.min(limit, projects.length),
    })
  } catch (error) {
    next(error)
  }
...
app.get('/api/projects/:projectId', async (req, res, next) => {
  try {
    ...
    const projectPath = path.resolve(projectsRoot, projectId, 'project.json')
    ...
    const projects = await listProjects({ limit: 200, light: false })
    const fullProject = projects.find((item) => item.id === projectId) || null
...
app.post('/api/run/merge-image-generate', upload.array('images'), async (req, res, next) => {
  ...
  try {
    ...
    const hasAngleShoePrelayoutReference = requiredRoles.has('angleShoePrelayout')
    const hasAngleFootSideReference = requiredRoles.has('angleFootSideReference')
    const hasAngle01FrontProductReference = requiredRoles.has('angle01FrontProduct')
    const hasAngle02InnerSideProductReference = requiredRoles.has('angle02InnerSideProduct')
    const hasAngle03InnerSideProductReference = requiredRoles.has('angle03InnerSideProduct')
    ...
    const extraProductFinalOverride = hasExtraProductReferences
      ...
      : ''
    const angle01TwoProductFinalOverride = hasAngle01FrontProductReference
      ? 'FINAL ANGLE 1 TWO-PRODUCT OVERRIDE: in angle 1 two-product mode, follow the angle-01 marked reference labels. The FRONT FOOT / lower-left planted shoe visible side is the INNER side at the medial ankle and must use Product image 2 / angle-1-inner-side-product-shoe. The REAR FOOT / upper-right lifted shoe visible side is the OUTER side at the lateral ankle and must use Product image 1 / main product-shoe reference. Product 2 is not an extra reference for the same shoe, not a hand-held/display shoe, and not a background/model/angle reference. If any generic wording says "product shoe image" or "product reference", read it as the assigned product image for each marked foot side: front-foot inner side uses Product 2, rear-foot outer side uses Product 1. Do not generate two same-side shoes, do not swap front/rear assignments, do not swap inner/outer roles, do not let Product 1 override the front-foot inner side, and do not let Product 2 override the rear-foot outer side.'
      : ''
    const angle02TwoProductFinalOverride = hasAngle02InnerSideProductReference
      ? 'FINAL ANGLE 2 TWO-PRODUCT OVERRIDE: in angle 2 two-product mode, Product image 1 / main product-shoe reference is used only for the two worn shoes middle-facing OUTER sides, and Product image 2 / angle-2-inner-side-product-shoe is used only for the far-left and far-right INNER sides. Product 2 is not an extra shoe, not a generic supplementary product view, and not a hand/foot/model/background reference. The final angle-2 image must keep the seated first-person crossed-leg pose: two shoes worn on both feet, middle-facing sides are outer sides, far outside edges are inner sides, close shoe-focused crop.'
      : ''
    const angle03TwoProductFinalOverride = hasAngle03InnerSideProductReference
      ? 'FINAL ANGLE 3 TWO-PRODUCT OVERRIDE: in angle 3 two-product mode, Product image 1 / main product-shoe reference is used only for the outside-facing outer shoe sides, and Product image 2 / angle-3-inner-side-product-shoe is used only for the middle-facing inner shoe sides. Product 2 is not an extra shoe, not a generic supplementary product view, and not a hand/foot/model/background reference. The final angle-3 image must keep two shoes tightly adjacent like the angle reference: inner side touches inner side in the center; outer sides face outward on the far left and far right. Do not generate feet, legs, ankles, toes, or worn shoes.'
      : ''
    const angle04TwoProductFinalOverride = hasAngle04GroundProductReference
      ? 'FINAL ANGLE 4 TWO-PRODUCT OVERRIDE: in angle 4 two-product mode, Product image 1 / main product-shoe reference is used only for the raised/lifted-foot worn shoe marked 图一 in the angle-04 reference. Product image 2 / angle-4-ground-foot-product-shoe is used only for the grounded/standing-foot worn shoe marked 图二 in the angle-04 reference. Product 2 is not an extra shoe, not a generic supplementary product view, and not a hand/foot/model/background reference. If any generic singular wording says "product shoe image" or "product reference", read it as the assigned product image for each foot role: raised/lifted foot uses Product 1, grounded/standing foot uses Product 2. Do not swap these roles and do not let Product 1 override the grounded-foot shoe.'
      : ''
    const angle05TwoProductFinalOverride = hasAngle05InnerSideProductReference
      ? 'FINAL ANGLE 5 TWO-PRODUCT OVERRIDE: in angle 5 two-product mode, Product image 1 / main product-shoe reference is used only for shoe OUTER sides, and Product image 2 / angle-5-inner-side-product-shoe is used only for shoe INNER sides. Assign the visible shoe side by ankle anatomy and the angle-05 marked reference: lateral malleolus / outer ankle uses Product 1 outer-side appearance; medial malleolus / inner ankle uses Product 2 inner-side appearance. Product 2 is not an extra reference for the same shoe, not a screen-left/screen-right shoe, not a hand-held/display shoe, and not a background/model/angle reference. If any generic wording says "product shoe image" or "product reference", read it as the assigned product image for each ankle side: outer ankle side uses Product 1, inner ankle side uses Product 2. Do not generate two same-side shoes, do not swap inner/outer roles, and do not let Product 1 override the inner-ankle side.'
      : ''
    ...
    const isHandAngleControl = hasAngleControlReference && /HAND ANGLE CONTROL|BUCKLE SIDE HARD LOCK BY CODE|buckleSide=/i.test(String(angleLayout || ''))
    ...
    const handAngleExposureInstruction = isHandAngleControl ? buildHandAngleExposureInstruction() : ''
    ...
    const conciseReferenceInstruction = hasAngleControlReference
      ? [
          ...
          hasAngleShoePrelayoutReference
            ? hasModelReference
              ? 'Image 1 = cleaned angle control layout guide; image 2 = hand-angle product shoe prelayout reference; image 3 = original uploaded angle reference; image 4 = background environment; image 5 = model outfit/body styling; image 6 = product shoe identity.'
              : 'Image 1 = cleaned angle control layout guide; image 2 = hand-angle product shoe prelayout reference; image 3 = original uploaded angle reference; image 4 = background environment; image 5 = product shoe identity.'
            : hasModelReference
          ...
          isHandAngleControl
            ? hasAngleShoePrelayoutReference
              ? 'First read the dynamic control image explanation below, then use the structured angle schema for pose, shoe count, worn/handheld/display roles, red clothing regions, camera view, crop focus, and S/B/R object coordinates. For the hand-angle channel, image 2 is the product-shoe prelayout visual hard reference for each S shoe toe/front direction, heel direction, rotation, placement, and scale. Also visually inspect image 3 original hand-angle reference only as an AI rough-pose recognition reference; do not let image 3 override the code-detected object count, roles, coordinates, foot count, shoe count, or S toe/heel direction.'
              : 'First read the dynamic control image explanation below, then use the structured angle schema for pose, shoe count, worn/handheld/display roles, red clothing regions, camera view, crop focus, and S/B/R object coordinates. For the hand-angle channel, also visually inspect image 2 as an AI rough-pose recognition reference to understand the broad action and gesture, but do not let image 2 override the code-detected object count, roles, coordinates, foot count, shoe count, or S toe/heel direction.'
          ...
          isHandAngleControl
            ? 'Use the background reference as the source of wall, floor, baseboard, texture, and environment structure only. For the hand-angle channel, do not let a beige/gray/dim background reference lower final exposure, reduce saturation, or create a dull gray cast; final lighting must follow the HAND-ANGLE EXPOSURE AND COLOR PRIORITY high-key commercial finish.'
    ...
    const realPhotoReferenceInstruction = hasRealPhotoAngleReference
      ...
      : ''
    const angleFootSideReferenceInstruction = hasAngleFootSideReference
      ? [
          'ANGLE 17 FOOT-SIDE VISUAL REFERENCE:',
          'An extra final reference image is provided only for angle 17 foot-side disambiguation.',
          'Use this extra reference only to decide the worn foot anatomy side, inner-ankle visibility, leg entry direction, and the worn shoe visible no-buckle/no-clasp side.',
          'Do not copy shoe style, color, material, buckle design, wall, floor, background, lighting, guide box, or scene from this extra reference.',
          'The angle mask still controls composition and placement; the product shoe reference still controls 100% of shoe identity; the background reference still controls the environment.',
          'For angle 17, the worn shoe visible side should match the extra reference: no buckle, no ring, no clasp, no strap fastener, no metal hardware on the visible side of the worn shoe.',
        ].join('\n')
      : ''
    const finalHardLayoutLock = hasAngleControlReference
      ? isSingleFootAngleControl
        ...
        : [
            ...
            isHandAngleControl ? 'HAND-ANGLE FINAL SHOE MARKER LOCK: the BUCKLE SIDE HARD LOCK BY CODE, every S buckleSide=... value, every orange buckle/clasp marker, every cyan TOE marker, every magenta HEEL marker, and every red heel-to-toe arrow from the hand-angle layout are final hard constraints, not optional hints.' : '',
            isHandAngleControl ? 'HAND-ANGLE PRIORITY ORDER: cyan TOE marker, magenta HEEL marker, red heel-to-toe arrow, S bbox, and S center are stronger than orange buckle/clasp side markers. If buckle/clasp placement conflicts with toe/heel direction or bbox, keep the toe/heel direction and bbox unchanged, then place the hardware on the matching outer side.' : '',
            isHandAngleControl ? 'HAND-ANGLE PER-SHOE FINAL DIRECTION LOCK: if the layout contains S1/S2/S3 FINAL DIRECTION LOCK lines, follow each line independently. For each S shoe, the product shoe toe/front must land at that S TOE endpoint and the heel must land at that S HEEL endpoint. The uploaded product image original direction, hand-holding pose, and left/right pairing must never reverse any S shoe toe/front direction.' : '',
            isHandAngleControl ? 'For the hand-angle channel, if the uploaded product shoe has a buckle, clasp, ring, strap fastener, buckle hole, strap anchor, or obvious side hardware, place that hardware on each S shoe near its own orange marker and on its declared buckleSide. If the product shoe has no such hardware, preserve the S toe/heel axis and ignore only the orange marker.' : '',
            isHandAngleControl ? 'For two visible S shoes in the hand-angle channel, the buckleSide values must read as opposite outer sides of a real left/right pair. If both buckles/clasps/rings appear on the same canvas side, the same shoe side, or the two shoes look like copied same-side duplicates, the result fails.' : '',
            isHandAngleControl ? 'Do not satisfy left/right pairing by moving only the buckle. The whole shoe body, opening, strap anchor, side seam, inner wall, outer wall, heel counter, and buckle side must read as the correct left/right paired shoe while preserving each S bbox, scale, perspective, cyan-to-magenta toe/heel axis, and toeDirection.' : '',
            isHandAngleControl && /HAND ANGLE EYE-LEVEL SIDE-VIEW CAMERA LOCK|cameraView=eye-level \/ side-view/i.test(String(angleLayout || '')) ? 'HAND-ANGLE FINAL CAMERA LOCK: cameraViewByCode says eye-level / side-view. Preserve the平视/侧面镜头 with side-profile legs/shoes and garment crop. Do not convert it into top-down, overhead, or first-person looking-down perspective.' : '',
            isHandAngleControl && /HAND ANGLE FIRST-PERSON DOWNWARD CAMERA LOCK|cameraView=first-person downward \/ top-down/i.test(String(angleLayout || '')) ? 'HAND-ANGLE FINAL CAMERA LOCK: cameraViewByCode says first-person downward / top-down POV. Preserve the俯视/第一视角往下拍镜头. Do not convert it into eye-level side-view or standing catalog perspective.' : '',
            isHandAngleControl ? 'HAND-ANGLE WORN-SHOE ROLE LOCK: use the S role map literally. Only S objects whose role is WORN_ON_FOOT may contain a foot, ankle, leg, toes, or sock. S objects whose role is HAND_HELD_OR_HAND_SUPPORTED_DISPLAY or DISPLAY_ONLY must remain bare product shoes with no foot/leg inside.' : '',
            isHandAngleControl ? 'HAND-ANGLE NO-WORN-SHOE OVERRIDE: if the layout says wornShoes=none, no S shoe may be worn by any foot. All S shoes must remain bare hand-held, hand-supported, or standalone display shoes. Do not create a try-on scene, do not put socks/toes/feet into any shoe, and do not let a blue/B foot visually enter a product shoe.' : '',
            isHandAngleControl ? 'HAND-ANGLE SINGLE-BLUE-FOOT LOCK: if the layout says blueFootLegCount=1 or blueLegFootRegionCount=1, generate exactly one visible foot/leg/ankle region and exactly the listed worn shoe(s). Do not add a second foot, second leg, second ankle, second sock, second toes, or second worn shoe from the model reference or clothing reference.' : '',
            isHandAngleControl ? 'HAND-ANGLE DETACHED-BLUE-FOOT LOCK: if a B foot/leg region has touchingShoes=none, it is detached from all product shoes. Keep it secondary/cropped as body posture only; it must not overlap, enter, touch, or wear any S shoe.' : '',
            isHandAngleControl ? 'HAND-ANGLE TOTAL-SHOE LOCK: if the layout says mainShoeObjectCount=2, the final photo must contain exactly two product shoes total: the listed hand-held/display shoe(s) plus the listed worn shoe(s). Do not add any extra shoe even if a normal model pose would have two feet.' : '',
            isHandAngleControl ? 'HAND-ANGLE AI ROUGH-POSE LOCK: use image 2 original uploaded hand-angle reference only to recognize the broad pose/action, such as hand holding or supporting display shoes, lower garment crop, body lean, camera feeling, and hand-to-shoe distance. This AI rough-pose reading is secondary to the S/B/G/R code map and must not add extra shoes, feet, legs, hands, clothing regions, worn-shoe actions, or change S/B/G/R coordinates.' : '',
            'Red clothing regions are only lower-body outfit support. They must not introduce the model face, head, portrait, torso, or full upper body.',
    ...
    const finalPrompt = [
      ...
      angleFootSideReferenceInstruction,
      !hasAngleControlReference ? mergeImageLockInstruction : '',
      isHandAngleControl
        ? hasAngleShoePrelayoutReference
          ? 'HAND-ANGLE REFERENCE USE OVERRIDE: image 1 cleaned hand-angle control is the code layout/count/coordinate map; image 2 product shoe prelayout is the visual hard reference for real product-shoe toe/front direction, heel direction, rotation, placement, and scale; image 3 original hand-angle reference is only rough pose/action verification. Images 1 and 3 are not color, exposure, contrast, saturation, lighting, or gray/black mood references. Keep the final composite bright, clean, high-key, and commercially retouched.'
          : 'HAND-ANGLE REFERENCE USE OVERRIDE: image 1 cleaned hand-angle control and image 2 original hand-angle reference are hard layout/pose/count/direction references only, not color, exposure, contrast, saturation, lighting, or gray/black mood references. Keep the final composite bright, clean, high-key, and commercially retouched even when those references contain black background or gray guide fills.'
        : '',
      hasAngleControlReference
        ? hasModelReference
          ? hasAngleShoePrelayoutReference
            ? hasAngle01FrontProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 product shoe prelayout as the hard visual reference for the two worn shoes placement and direction. Use image 3 original angle reference only as secondary pose verification. Use image 4 background for environment. Use image 5 model reference only for outfit/body styling. For angle 1 two-product mode: use image 6 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 7 angle-1-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not use image 6 for inner-ankle sides.'
              : hasAngle05InnerSideProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 product shoe prelayout as the hard visual reference for the two worn shoes placement and direction. Use image 3 original angle reference only as secondary pose verification. Use image 4 background for environment. Use image 5 model reference only for outfit/body styling. For angle 5 two-product mode: use image 6 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 7 angle-5-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not use image 6 for inner-ankle sides.'
              : hasAngle03InnerSideProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 product shoe prelayout as the hard visual reference for the two close shoes placement and direction. Use image 3 original angle reference only as secondary pose verification. Use image 4 background for environment. Use image 5 model reference only for outfit/body styling. For angle 3 two-product mode: use image 6 main product-shoe reference only for the outside-facing outer shoe sides, and use image 7 angle-3-inner-side-product-shoe reference / Product 2 only for the two middle-facing inner shoe sides where the shoes touch. Do not use image 6 for the middle-facing inner sides.'
              : 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 product shoe prelayout as the hard visual reference for each product shoe toe/front direction, heel direction, rotation, placement, and scale. Use image 3 original angle reference only as secondary pose verification. Use image 4 background for environment. Use image 5 model reference only for outfit/body styling. Use image 6 product reference for shoe identity.'
            : hasAngle01FrontProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 original angle reference only as secondary verification. Use only image 3 background for environment. Use image 4 model reference only for outfit/body styling. For angle 1 two-product mode: use image 5 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 6 angle-1-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not use image 5 for inner-ankle sides.'
              : hasAngle05InnerSideProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 original angle reference only as secondary verification. Use only image 3 background for environment. Use image 4 model reference only for outfit/body styling. For angle 5 two-product mode: use image 5 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 6 angle-5-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not use image 5 for inner-ankle sides.'
              : hasAngle03InnerSideProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 original angle reference only as secondary verification. Use only image 3 background for environment. Use image 4 model reference only for outfit/body styling. For angle 3 two-product mode: use image 5 main product-shoe reference only for the outside-facing outer shoe sides, and use image 6 angle-3-inner-side-product-shoe reference / Product 2 only for the two middle-facing inner shoe sides where the shoes touch. Do not use image 5 for the middle-facing inner sides.'
              : 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 original angle reference only as secondary verification. Use only image 3 background for environment. Use image 4 model reference only for outfit/body styling. Use image 5 product reference for shoes.'
          : hasAngleShoePrelayoutReference
            ? hasAngle01FrontProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 product shoe prelayout as the hard visual reference for the two worn shoes placement and direction. Use image 3 original angle reference only as secondary pose verification. Use image 4 background for environment. For angle 1 two-product mode: use image 5 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 6 angle-1-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not use image 5 for inner-ankle sides.'
              : hasAngle05InnerSideProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 product shoe prelayout as the hard visual reference for the two worn shoes placement and direction. Use image 3 original angle reference only as secondary pose verification. Use image 4 background for environment. For angle 5 two-product mode: use image 5 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 6 angle-5-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not use image 5 for inner-ankle sides.'
              : hasAngle03InnerSideProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 product shoe prelayout as the hard visual reference for the two close shoes placement and direction. Use image 3 original angle reference only as secondary pose verification. Use image 4 background for environment. For angle 3 two-product mode: use image 5 main product-shoe reference only for the outside-facing outer shoe sides, and use image 6 angle-3-inner-side-product-shoe reference / Product 2 only for the two middle-facing inner shoe sides where the shoes touch. Do not use image 5 for the middle-facing inner sides.'
              : 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 product shoe prelayout as the hard visual reference for each product shoe toe/front direction, heel direction, rotation, placement, and scale. Use image 3 original angle reference only as secondary pose verification. Use image 4 background for environment. Use image 5 product reference for shoe identity.'
            : hasAngle01FrontProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 original angle reference only as secondary verification. Use only image 3 background for environment. For angle 1 two-product mode: use image 4 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 5 angle-1-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not use image 4 for inner-ankle sides.'
              : hasAngle05InnerSideProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 original angle reference only as secondary verification. Use only image 3 background for environment. For angle 5 two-product mode: use image 4 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 5 angle-5-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Do not use image 4 for inner-ankle sides.'
              : hasAngle03InnerSideProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 original angle reference only as secondary verification. Use only image 3 background for environment. For angle 3 two-product mode: use image 4 main product-shoe reference only for the outside-facing outer shoe sides, and use image 5 angle-3-inner-side-product-shoe reference / Product 2 only for the two middle-facing inner shoe sides where the shoes touch. Do not use image 4 for the middle-facing inner sides.'
              : 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 cleaned angle control as the hard layout reference. Use image 2 original angle reference only as secondary verification. Use only image 3 background for environment. Use image 4 product reference for shoes.'
        : hasRealPhotoAngleReference
          ? hasModelReference
          ? hasAngle01FrontProductReference
            ...
            ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 real photo angle reference as the hard camera/pose/composition reference. Use only image 2 background for environment. Use image 3 model reference only for outfit, body-limb styling, skin tone, and lower-body proportions, excluding background, shoes, props, and watermark. For angle 6 two-product mode: use image 4 main product-shoe reference only for the worn shoe, and use image 5 angle-6-hand-held-product-shoe reference / 产品2 only for the hand-held display shoe.'
            ...
            : hasAngle01FrontProductReference
              ...
              : hasAngle06HandProductReference
              ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use image 1 real photo angle reference as the hard camera/pose/composition reference. Use only image 2 background for environment. For angle 6 two-product mode: use image 3 main product-shoe reference only for the worn shoe, and use image 4 angle-6-hand-held-product-shoe reference / 产品2 only for the hand-held display shoe.'
          ...
          : hasModelReference
          ? hasAngle01FrontProductReference
            ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use the uploaded references as hard references. Match image 1 angle-mask for composition, walking pose, two worn shoes, yellow-region shoe count/placement/angle/scale, and camera. Use only image 2 background for environment. Use image 3 model reference only for outfit, body-limb styling, skin tone, and lower-body proportions, excluding background, shoes, props, and watermark. For angle 1 two-product mode: use image 4 main product-shoe reference only for the rear/upper-right lifted shoe visible OUTER side on the lateral ankle, and use image 5 angle-1-inner-side-product-shoe reference / Product 2 only for the front/lower-left planted shoe visible INNER side on the medial ankle. Follow the angle-01 marked labels. The front-foot inner side must not fall back to image 4.'
            : hasAngle05InnerSideProductReference
            ...
            : hasAngle06HandProductReference
            ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use the uploaded references as hard references. Match image 1 angle-mask for composition, pose, blue-region limb identity, yellow-region shoe count/placement/angle/scale, and camera. Use only image 2 background for environment. Use image 3 model reference only for outfit, body-limb styling, skin tone, and lower-body proportions, excluding background, shoes, props, and watermark. For angle 6 two-product mode: use image 4 main product-shoe reference only for the worn shoe, and use image 5 angle-6-hand-held-product-shoe reference / 产品2 only for the hand-held display shoe. The hand-held display shoe must not fall back to image 4.'
            : 'No requirement analysis or plan is needed. Directly generate one final composite image. Use the uploaded references as hard references. Match image 1 angle-mask for composition, pose, blue-region limb identity, yellow-region shoe count/placement/angle/scale, and camera. Use only image 2 background for environment. Use image 3 model reference only for outfit, body-limb styling, skin tone, and lower-body proportions, excluding background, shoes, props, and watermark. Use only image 4 product reference for shoes.'
          : hasAngle01FrontProductReference
            ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use the uploaded references as hard references. Match image 1 angle-mask for composition, walking pose, two worn shoes, yellow-region shoe count/placement/angle/scale, and camera. Use only image 2 background for environment. For angle 1 two-product mode: use image 3 main product-shoe reference only for the rear/upper-right lifted shoe visible OUTER side on the lateral ankle, and use image 4 angle-1-inner-side-product-shoe reference / Product 2 only for the front/lower-left planted shoe visible INNER side on the medial ankle. Follow the angle-01 marked labels. The front-foot inner side must not fall back to image 3.'
            : hasAngle05InnerSideProductReference
            ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use the uploaded references as hard references. Match image 1 angle-mask for composition, angle-05 pose, two worn shoes, yellow-region shoe count/placement/angle/scale, and camera. Use only image 2 background for environment. For angle 5 two-product mode: use image 3 main product-shoe reference only for shoe OUTER sides on lateral/outer ankles, and use image 4 angle-5-inner-side-product-shoe reference / Product 2 only for shoe INNER sides on medial/inner ankles. Assign by ankle anatomy and the angle-05 inner/outer labels, not screen left/right alone. The inner-ankle side must not fall back to image 3.'
            : hasAngle03InnerSideProductReference
            ...
            ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use the uploaded references as hard references. Match image 1 angle-mask for composition, pose, blue-region limb identity, yellow-region shoe count/placement/angle/scale, and camera. Use only image 2 background for environment. For angle 4 two-product mode: use image 3 main product-shoe reference only for the raised/lifted-foot worn shoe marked 图一, and use image 4 angle-4-ground-foot-product-shoe reference / Product 2 only for the grounded/standing-foot worn shoe marked 图二. The grounded-foot shoe must not fall back to image 3.'
            : hasAngle06HandProductReference
            ? 'No requirement analysis or plan is needed. Directly generate one final composite image. Use the uploaded references as hard references. Match image 1 angle-mask for composition, pose, blue-region limb identity, yellow-region shoe count/placement/angle/scale, and camera. Use only image 2 background for environment. For angle 6 two-product mode: use image 3 main product-shoe reference only for the worn shoe, and use image 4 angle-6-hand-held-product-shoe reference / 产品2 only for the hand-held display shoe. The hand-held display shoe must not fall back to image 3.'
            : 'No requirement analysis or plan is needed. Directly generate one final composite image. Use the uploaded references as hard references. Match image 1 angle-mask for composition, pose, blue-region limb identity, yellow-region shoe count/placement/angle/scale, and camera. Use only image 2 background for environment. Use only image 3 product reference for shoes.',
      extraProductFinalOverride,
      angle01TwoProductFinalOverride,
      angle02TwoProductFinalOverride,
      angle03TwoProductFinalOverride,
